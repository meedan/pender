    ...
    def send_to_archive_org(url, key_id, _supported = nil)
      handle_archiving_exceptions('archive_org', { url: url, key_id: key_id }) do
        ...
        if body['job_id']
          ...
          if body['message']&.include?('429 Too Many Requests')
            PenderSentry.notify(
              Pender::Exception::RateLimitExceeded.new(body['message']),
              url: url,
              response_body: body
            )
          elsif body['message']&.include?('The same snapshot') || body['status_ext'] == 'error:too-many-daily-captures'
            PenderSentry.notify(
