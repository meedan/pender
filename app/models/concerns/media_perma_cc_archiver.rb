module MediaPermaCcArchiver
  ...
  included do
    Media.declare_archiver('perma_cc', [/^.*$/], :only, false)
  end
  ...
  module ClassMethods
    ...
    def send_to_perma_cc(url, key_id, attempts = 1, response = nil)
      ...
      Rails.logger.info "[Archiver Perma.cc] Sending #{url} to Perma.cc: { response_code: #{response.code}, response_body: #{response.body} }"
