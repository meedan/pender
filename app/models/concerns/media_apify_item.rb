module MediaApifyItem
  ...
  Media.class_eval do
    ...
    def self.invalid_response?(response)
      ...
      return true unless %w[200 201].include?(response.code)
      return true if response.body.blank?
      return true if response.body.include?("This content isn't available")
      false
    end
    ...
    def self.handle_response(response, apify_url)
      ...
      raise Pender::Exception::ApifyResponseError if invalid_response?(response)

      parsed_response
    rescue Pender::Exception::ApifyResponseError, JSON::ParserError => error
    ...
    def self.handle_apify_error(error, apify_url, parsed_response)
      ...
      apify_error_class = apify_error_class(apify_response_error_description)
      apify_error = apify_error_class.new(error.message)
      ...
      Rails.logger.warn(
        level: 'WARN',
        message: '[Parser] Could not process Apify request',
        apify_url: apify_url,
        apify_response_url: apify_response_url,
        error_class: apify_error
        )
    end

    def self.apify_error_class(apify_response_error_description)
      return Pender::Exception::ApifyError if apify_response_error_description.nil?

      case
      when apify_response_error_description.include?("Apify data not found or link is inaccessible")
        Pender::Exception::ApifyDataNotFoundError
      when apify_response_error_description.include?("By launching this job you will exceed the memory limit")
        Pender::Exception::ApifyActorMemoryError
      when apify_response_error_description.include?("This content isn't available")
        Pender::Exception::ApifyDataNotAvailableError
      else
        Pender::Exception::ApifyError
      end
    end
    ...
    def self.parsed_apify_response(parsed_response)
      ...
      apify_response_error_description = apify_response['errorDescription'].presence || apify_response["message"] || apify_response.dig("error", "message")
