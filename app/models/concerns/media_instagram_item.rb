module MediaInstagramItem
  ...
  def data_from_instagram_item
    ...
    handle_exceptions(self, StandardError) do
      self.get_instagram_item_data(id.to_s)
      self.data.merge!(external_id: id)
      data = self.data
      return if data.dig('raw', 'graphql', 'error')
      username = self.get_instagram_username_from_graphql_data
  ...
  def get_instagram_username_from_graphql_data
  ...
  def get_instagram_item_picture_from_data
    self.data.dig('raw', 'graphql', 'image_versions2', 'candidates', 0, 'url') ||
      self.data.dig('raw', 'graphql', 'carousel_media', 0, 'image_versions2', 'candidates', 0, 'url')
  end

  def get_instagram_item_data(id)
    begin
  ...
  def get_instagram_graphql_data(url)
    ...
    self.ignore_url?(location) ? (raise StandardError.new("Page unavailable, encountered #{self.unavailable_page}")) : self.get_instagram_graphql_data(location)
  end
