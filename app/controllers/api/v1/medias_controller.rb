  ...
  module V1
    class MediasController < Api::V1::BaseApiController
      ...
      def index
        ...
        (render_uncached_media and return) if @refresh || @store.read(:json).nil?
      ...
      def render_as_html
        begin
          if @refresh || !@store.exist?(:html)
            save_cache
          end
          render text: @store.read(:html), status: 200
        rescue
