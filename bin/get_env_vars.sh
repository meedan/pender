...
if (( $? != 0 )); then
  ...
  SSM_NAMES=$(aws ssm get-parameters-by-path --region $REGION --path /local/pender/ --recursive --with-decryption --output text --query "Parameters[].[Name]")
  ...
  for NAME in $SSM_NAMES; do
    ...
    echo "$VARNAME: $VALUE" >> config/config-test.yml
  done
fi
